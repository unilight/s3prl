...
FS = 16000
OBJ_EVAL_START_EPOCH = 4000 # should this be in config?
...
class DownstreamExpert(nn.Module):
    def __init__(self, upstream_dim, upstream_rate, downstream_expert, expdir, trgspk, **kwargs):
        ...
        # load f0min and f0 max for objective evaluation
        with open(self.datarc["f0_path"], 'r') as f:
            f0_all = yaml.load(f, Loader=yaml.FullLoader)
        self.f0min = f0_all[self.trgspk]["f0min"]
        self.f0max = f0_all[self.trgspk]["f0max"]
        ...
        if os.path.exists(stats_path):
            ...
            for _, lmspc, _ in self.train_dataset:
    ...
    def log_records(self, split, records, logger, global_step, batch_ids, total_batch_num, **kwargs):
        ...
        if split in ["dev", "test"]:
            ...
            for i, wav_path in enumerate(records["wav_paths"]):
                ...
                wav = records["wavs"][i] # ground truth target wav
                ...
                # only do MCD & F0 related objective evaluation with dev set and after model reach certain epoch (=quality)
                if split == "dev" and global_step >= OBJ_EVAL_START_EPOCH:
                    mcd, f0rmse, f0corr, ddur = calculate_mcd_f0(y, wav, self.fs, self.f0min, self.f0max)
                    mcds.append(mcd)
                    f0rmses.append(f0rmse)
                    f0corrs.append(f0corr)
                    ddurs.append(ddur)

        if split == "dev" and global_step >= OBJ_EVAL_START_EPOCH:
            mcd = np.array(mcds).mean()
            f0rmse = np.array(f0rmses).mean()
            f0corr = np.array(f0corrs).mean()
            ddur = np.array(ddurs).mean()
            print(f'{split} loss: {loss:.6f}, MCD: {mcd:.2f}, F0RMSE: {f0rmse:.3f}, F0CORR: {f0corr:.3f}, DDUR: {ddur:.3f}')
        else:
